...
import { safeLaunchOptions } from './config';
import type { LaunchOptions } from './types';

export async function launch(options: LaunchOptions = {}) {
  const maxAttempts = 5;
  const _options = { ...safeLaunchOptions(options) };
  let attempt = 1;

  let browser: puppeteer.Browser | undefined;

  console.log(`puppeteer: launching with settings: ${JSON.stringify(_options)}`);

  while (!browser) {
    try {
      browser = await puppeteer.launch(_options);
